app.controller(
  ...
  function ($scope, $http, $window, $routeParams, $location, authService) {
    ...
    function loadContent() {
      ...
      // Load read statuses
      $http
        .get(`http://localhost:5000/api/read-status/status/${courseId}`, {
          headers: { Authorization: "Bearer " + token },
        })
        .then(function (res) {
    ...
    $scope.markAsRead = function (contentId) {
      $http
        .post(
          "http://localhost:5000/api/read-status/mark",
          {
            courseId,
            contentId,
          },
